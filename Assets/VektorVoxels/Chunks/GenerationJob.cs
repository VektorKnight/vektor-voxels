        ...
        public void Execute() {
            ...
            for (var z = 0; z < dimensions.x; z++) {
                for (var x = 0; x < dimensions.x; x++) {
                    _chunk.VoxelData[VoxelUtility.VoxelIndex(x, 0, z, dimensions)] = VoxelTable.ById(1).GetDataInstance();
                    _chunk.HeightMap[VoxelUtility.HeightIndex(x, z, dimensions.x)] = new HeightData(0, true);
                }
            }

            if (true) {
                _chunk.VoxelData[VoxelUtility.VoxelIndex(4, 1, 6, dimensions)] = VoxelTable.ById((uint)(11 + (_chunk.ChunkId.x % 2))).GetDataInstance();
